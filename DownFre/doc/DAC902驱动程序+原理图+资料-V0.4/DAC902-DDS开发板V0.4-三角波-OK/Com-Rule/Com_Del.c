...
{
	u16 re;
	re = Com_dat[3]*256*256*256+Com_dat[2]*256*256+Com_dat[1]*256+Com_dat[0];
	Com_dat+=4;
	return re;
}
...
u8 AD9833_deal(void)
{
	switch(Command)
	{
		case FRE:AD9833_SetFrequencyQuick(Fre_decoding(),AD9833_OUT_SINUS);break;
		case sine:AD9833_SetFrequencyQuick(Fre_decoding(),AD9833_OUT_SINUS);break;
		case Square:AD9833_SetFrequencyQuick(Fre_decoding(),AD9833_OUT_MSB);break;
		case Triangle:AD9833_SetFrequencyQuick(Fre_decoding(),AD9833_OUT_TRIANGLE);break;
...
u8 AD9851_deal(void)
{
	switch(Command)
	{
		...
		case sine:AD9851_Setfq(Fre_decoding());break;
////		case Square:break;
////		case Triangle:break;
//		case FSK:break;
//		case BPSK:break;
...
u8 AD9854_deal(void)
{
	...
	{
		case FRE:break;
		case sine:	if(DDS_mod!=sine)AD9854_Init();				DDS_mod=sine;		AD9854_SetSine_double(Fre_decoding(),Shape_decoding());break;
		case Square:break;
		case Triangle:break;
		...
		case OSK:		if(DDS_mod!=OSK)	AD9854_InitOSK();		DDS_mod=OSK;		AD9854_SetOSK(Fre_decoding());break;
...
//		case OSK:		if(DDS_mod!=OSK)	AD9854_InitOSK();		DDS_mod=OSK;		AD9854_SetOSK(*Com_dat);break;
		...
		case FM:		if(DDS_mod!=FM)			break;
//		case RFSK:	if(DDS_mod!=RFSK)	AD9854_InitRFSK();	DDS_mod=RFSK;		AD9854_SetRFSK(Fre_decoding(),Fre_decoding(),Fre_decoding(),Fre_decoding());break;
		case Sweep:	if(DDS_mod!=Sweep)											DDS_mod=Sweep;			Linear_Sweep(Fre_decoding(),Fre_decoding(),Fre_decoding(),Phase_decoding(),Fre_decoding(),Phase_decoding(),Phase_decoding());break;
		case parrel:break;//void 																								Linear_Sweep(double  MinFreq, double MaxFreq, double  UpStepFreq, u8 UpStepTime, double  DownStepFreq, u8 DownStepTime,    ScanMode      Mode）;        //0向下扫频模式，1向上扫频模式，2双边扫频
		case serial:break;
		default:return ERROR;
	}
	return SUCCESS;
...
u8 DDS_Chose(u8 DDS)
{
		switch(DDS)
		{
			...
			case AD9833:DDS_deal=AD9833_deal;break;
			default:return ERROR;
		}
		Chose_DDS_function(DDS);
		return SUCCESS;
}
u8 Change_DDS(u8 DDS)
{
	static u8 DDSBuf=0;
	u8 err=SUCCESS;
	if(DDSBuf!=DDS)err=DDS_Chose(DDS);
	DDSBuf=DDS;
	return err;
}
u8 DDS_DEAL(void)
{
		...
		{
			Get_Msg_mark=0;
			if(Change_DDS(DDS_model)==SUCCESS)
			{
				DDS_deal();
				return SUCCESS;
			}
